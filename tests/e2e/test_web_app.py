...
def test_register(client):
    ...
    response = client.post(
        ...
        data={'username': 'wpking1', 'password': 'Infosys2020'}
    )
...
@pytest.mark.parametrize(('username', 'password', 'message'), (
        ...
        ('wpking', 'Test#6^0', b'Your username is already taken - please supply another'),
))
...
def test_register_with_invalid_input(client, username, password, message):
    client.post(
        '/authentication/register',
        data={'username': 'wpking', 'password': 'Infosys2020'}
    )
...
def test_index(client):
    ...
    assert b'Code & Chill' in response.data
...
def test_movies_without_rank(client):
    ...
    assert b'Dan Trachtenberg' in response.data
    assert b'10 Cloverfield Lane' in response.data
...
def test_movies_with_rank(client):
    ...
    assert b'3 Days to Kill' in response.data


def test_movies_with_genre(client):
    # Check that we can retrieve the movies page.
    response = client.get('/movies_by_genre?genre=Thriller')
    assert response.status_code == 200

    assert b'Thriller Movies' in response.data
